...
# input_model_file="./deep-interact/assets/checkpoint.pt"
input_model_file="./deep-interact/assets/pretrain_PCQM4Mv2_e50_l8_bsz1024_mha_complete.pth"
# input_model_file="./deep-interact/assets/pretrain_QM9_e100_l8_bsz1024_outer_test_complete.pth"
output_model_name=finetune_${finetuning_dataset}_from_${pretraining_dataset}_gin_schnet_L${blocks}_test_outer

PYTHONPATH='.' python3 runners/finetune_QM9_deep_interact.py --input_data_dir=data --dataset=QM9 --epochs 1000 --input_model_file $input_model_file --output_model_dir ./deep-interact/assets  --lr_scheduler CosineAnnealingLR --batch_size $bsz --verbose --device $device --num_workers 8 --mode method --model_3d SchNet --model_2d $model_2d --gnn_type $model_2d --num_interaction_blocks $blocks --output_model_name $output_model_name --dropout_ratio 0.0 --interaction_rep_2d mean --interaction_rep_3d mean --interaction_agg cat --final_pool cat --mode method --emb_dim 300 --lr 1e-4 --initialization glorot_normal --input_model_file $input_model_file --save_config --verbose --diff_interactor_per_block --task=$task --interactor_type $interactor_type --interactor_activation Swish --residual