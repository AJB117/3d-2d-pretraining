...
from util import get_random_indices
...
def QM9_50k_split(dataset, task_idx=None, null_value=0, seed=123, smiles_list=None):
    if task_idx is not None:
        # filter based on null values in task_idx
        # get task array
        y_task = np.array([data.y[task_idx].item() for data in dataset])
    ...
    Nmols = 133885 - 3054
    all_idx = get_random_indices(Nmols, seed)
...
def atom3d_LBA_split(dataset, data_root, year):
    json_file = os.path.join(
        data_root, "processed", "pdb_id2data_id_{}.json".format(year)
    )
    f = open(json_file, "r")
    ...
    def load_data_id(mode):
        pdb_id_file = os.path.join(
            data_root, "processed", "targets", "{}.txt".format(mode)
        )
