...
def do_3D_masking(args, batch, model, mu, sigma):
    ...
    if args.model_3d == "SchNet":
        _, molecule_3D_repr_02 = model(
            x_02, positions_02, batch.batch, return_latent=True
        )
    elif args.model_3d == "PaiNN":
        _, molecule_3D_repr_02 = model(
            x_02, positions_02, batch.radius_edge_index, batch.batch, return_latent=True
        )
    ...
    distance_01 = torch.sqrt(torch.sum((u_pos_01 - v_pos_01) ** 2, dim=1)).unsqueeze(
        1
    )  # (num_edge, 1)
...
def train(args, molecule_model_2D, device, loader, optimizer):
    ...
    for step, batch in enumerate(l):
        ...
        if args.SDE_coeff_generative_2Dto3D > 0:
            SDE_loss_2Dto3D_result = SDE_2Dto3D_model(
                node_2D_repr, batch, anneal_power=args.SDE_anneal_power
            )
        ...
        if args.SDE_coeff_generative_3Dto2D > 0:
            SDE_loss_3Dto2Dx, SDE_loss_3Dto2Dadj = SDE_3Dto2D_model(
                node_3D_repr,
                batch,
                reduce_mean=reduce_mean,
                continuous=True,
                train=True,
                anneal_power=args.SDE_anneal_power,
            )
    ...
    print(
        "CL Loss: {:.5f}\tCL Acc: {:.5f}\t\tSDE 2Dto3D Loss: {:.5f}\tSDE 3Dto2D Loss: {:.5f}".format(
...
if __name__ == "__main__":
    ...
    if args.SDE_2Dto3D_model == "SDEModel2Dto3D_01":
        SDE_2Dto3D_model = SDEModel2Dto3D_01(
            ...
            hidden_dim=args.hidden_dim_2Dto3D,
            beta_min=args.beta_min_2Dto3D,
            beta_max=args.beta_max_2Dto3D,
    ...
    if args.SDE_3Dto2D_model == "SDEModel3Dto2D_node_adj_dense":
        SDE_3Dto2D_model = SDEModel3Dto2D_node_adj_dense(
            ...
            emb_dim=args.emb_dim,
            num_linears=3,
            beta_min=args.beta_min_3Dto2D,
            beta_max=args.beta_max_3Dto2D,
            num_diffusion_timesteps=args.num_diffusion_timesteps_3Dto2D,
            SDE_type=args.SDE_type_3Dto2D,
            num_class_X=node_class,
            noise_on_one_hot=args.noise_on_one_hot,
        ).to(device)
    ...
    model_param_group.append(
        {"params": SDE_2Dto3D_model.parameters(), "lr": args.lr * args.gnn_2d_lr_scale}
    )
    model_param_group.append(
        {"params": SDE_3Dto2D_model.parameters(), "lr": args.lr * args.gnn_3d_lr_scale}
    )
